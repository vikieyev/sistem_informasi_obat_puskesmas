...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM products where nama_toko= :nama_toko ORDER BY qty_sold DESC");
			...
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM products where qty < 10 and nama_toko= :nama_toko ORDER BY product_id DESC");
				$result->bindParam(':nama_toko', $_SESSION['SESS_FIRST_NAME']); //tambahan nama toko//
				$result->execute();
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				$result = $db->prepare("SELECT *, price * qty as total FROM products where nama_toko = :nama_toko ORDER BY product_id DESC");
