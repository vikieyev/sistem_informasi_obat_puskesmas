...
<center>
	<strong>
		Tanggal Terima Obat : <input type="text" style="width: 223px; padding:14px;" name="d1" class="tcal" value="<?php echo date("m/d/Y",strtotime(' -1 day')); ?>" /> 
	</strong>
...
Tanggal Terima Obat: &nbsp;<?php echo date("d-M-Y", strtotime($_GET['d1'] ));?>
</div>
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			...
			<?php
				...
				$result = $db->prepare("SELECT * FROM pkm_kedundung WHERE tgl_terima = :a ORDER by nama_obat ASC ");
	...
	<tbody>
			...
			<?php
				...
				$result = $db->prepare("SELECT * FROM master_obat WHERE tanggal_terima = :a  ORDER by nama_obat ASC ");
			...
			?>
						...
						<tr class="record">
							...
							<td><?php $no_obat =$row['nama_obat']; echo $row['jumlah_terima']; ?></td>
								...
								<?php
								include('../connect.php');
								$result2 = $db->prepare("SELECT * FROM pkm_kedundung where tgl_terima = :a and nama_obat = :no_obat ORDER by nama_obat ASC ");
								$result2->bindParam(':no_obat', $no_obat);
								$result2->bindParam(':a', $d1);
								//$result2->bindParam(':b', $d2);
								$result2->execute();
								
								$result3 = $db->prepare("SELECT * FROM pkm_kedundung where tgl_terima = :a and nama_obat = :no_obat ORDER by nama_obat ASC ");
