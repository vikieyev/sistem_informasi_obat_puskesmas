...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT * FROM sales WHERE date BETWEEN :a AND :b and nama_toko= :nama_toko ORDER by transaction_id DESC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(amount) FROM sales WHERE date BETWEEN :a AND :b and nama_toko= :nama_toko ");
				$results->bindParam(':a', $d1);
				$results->bindParam(':b', $d2);
				$results->bindParam(':nama_toko', $_SESSION['SESS_FIRST_NAME']);
				$results->execute();
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php 
				$resultia = $db->prepare("SELECT sum(profit) FROM sales WHERE date BETWEEN :c AND :d and nama_toko= :nama_toko ");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->bindParam(':nama_toko', $_SESSION['SESS_FIRST_NAME']);
				$resultia->execute();
